...
import { ModalCustom } from "../tour/modal";
...
const Cart = (props: IProps) => {
  ...
  const handleRemove = async (cartItem: ITourResponse) => {
    ...
    const items: ItemResponse[] = cartCurrent.item.filter(
      (i) => i.tour_id !== cartItem.id
    );
      ...
      {cartItems?.map((cartItem, index) => (
        <>
          <Row className="ctn-item__Row">
            <Col lg={3}>
              <Link href={`/tour/${cartItem.id} `}>
                <img
                  className="img-fluid"
                  src={`/img/${cartItem.imageUrls[0]}`}
                />
              </Link>
            </Col>

            <Col lg={3}>
              <div className="tour-code">Mã Tour:{" " + cartItem.code}</div>
              <Link
                href={`/tour/${cartItem.id} `}
                className="custom-hover-primary"
              >
                <h1 className="title h5 mt-1">{cartItem.title}</h1>
              </Link>
            </Col>
            <Col lg={6}>
              <Row>
                  ...
                  <Dropdown>
                    ...
                    <Dropdown.Menu>
                      <Button
                        ...
                        variant="danger"
                        onClick={() => {
                          handleShowBooking(cartItem.title, cartItem.id);
                        }}
                      >
                        Đặt ngay
                      </Button>
